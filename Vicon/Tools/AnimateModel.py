...
class AnimateModel:
    def __init__(
        self,
        window_title: str = "3D Marker Animation",
    ...
    def import_markers(self, data: dict):
        ...
        if all(length == length_list[0] for length in length_list):
            ...
            if self._length != length_list[0]:
                raise Exception(
                    "Error in AnimateModel: Marker sizes are not all the same"
                )
        else:
