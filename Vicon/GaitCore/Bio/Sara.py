...
class Sara:
    ...
    def _analyze_data(self):
        ...
        if len(self._data.get("X")) != len(self._data.get("Y")) or len(
            self._data.get("Y")
        ) != len(self._data.get("Z")):
            print("Error in Sara: position variables not the same size")
            # TODO: Throw error

        for x, y, z in zip(
            self._data.get("X", {}).get("data"),
        ...
        self._location = core.PointArray.PointArray(
            self.x_array, self.y_array, self.z_array
        )

        # Check to make sure all units are the same, and set units
        if self._data.get("X", {}).get("unit") == self._data.get("Y", {}).get(
            "unit"
        ) and self._data.get("X", {}).get("unit") == self._data.get("Z", {}).get(
            "unit"
        ):
